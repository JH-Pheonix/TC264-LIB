...
void tsl1401_send_data(uart_index_enum uart_n, uint8 index)
{
    ...
    for (i = 0; i < TSL1401_DATA_LEN; i++)
    {
        ...
        {
        case ADC_8BIT:
            uart_write_byte(uart_n, (uint8)(tsl1401_data[index][i]));
            break;
        case ADC_10BIT:
            uart_write_byte(uart_n, (uint8)(tsl1401_data[index][i] >> 2));
            break;
        case ADC_12BIT:
            uart_write_byte(uart_n, (uint8)(tsl1401_data[index][i] >> 4));
            break;
        }
    }
}
