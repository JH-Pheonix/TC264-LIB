...
IFX_ALIGN(256) DMA_LINK dma_link_list_2;
...
uint8 dma_init_2 (IfxDma_ChannelId dma_ch, uint8 *source_addr, uint8 *destination_addr, exti_pin_enum exti_pin, exti_trigger_enum trigger, uint32 dma_count, uint8 isr_prio)
{
    ...
    else
    {
        ...
        cfg.operationMode               = IfxDma_ChannelOperationMode_continuous;
         ...
         cfg.shadowAddress               = IFXCPU_GLB_ADDR_DSPR(IfxCpu_getCoreId(), (unsigned)&dma_link_list_2.linked_list[1]);
    ...
    if(1 < list_num)
    {
        ...
        while(i < list_num)
        {
            ...
            if(i == (list_num - 1))
            {
                ...
                cfg.shadowAddress       = IFXCPU_GLB_ADDR_DSPR(IfxCpu_getCoreId(), (uint32)&dma_link_list_2.linked_list[0]);
            ...
            else
            {
                ...
                cfg.shadowAddress       = IFXCPU_GLB_ADDR_DSPR(IfxCpu_getCoreId(), (uint32)&dma_link_list_2.linked_list[i+1]);
            ...
            IfxDma_Dma_initLinkedListEntry((void *)&dma_link_list_2.linked_list[i], &cfg);
        ...
        IfxDma_Dma_getSrcPointer(&dma_link_list_2.channel)->B.CLRR = 1;
